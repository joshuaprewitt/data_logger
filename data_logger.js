   ...
   checkActivePlugin: function() {
      ...
      if (window.location.hash === '#data_logger') {
         var setActivePluginCallback = function(viewspaceDom) {
            ...
            element.setAttribute('src', '/packages/local/data_logger/index.html');
         ...
         Skyline.Api.setActivePlugin('data_logger', null, 'data_logger.pluginTitle', setActivePluginCallback, this);
      }
   }
});

window.addEventListener('hashchange', data_logger.checkActivePlugin);

// Check if the page loaded with the hash.
data_logger.checkActivePlugin();